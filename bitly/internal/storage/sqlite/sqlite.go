...
func (s *Storage) SaveURL(urlToSave string, alias string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%s: failed to prepare the statement: %w", op, err)
	}
	...
	if err != nil {
		...
		return 0, fmt.Errorf("%s: failed to execute the statement: %w", op, err)
	}

	id, err := res.LastInsertId()
	if err != nil {
		return 0, fmt.Errorf("%s: failed to get the last insert ID: %w", op, err)
	}
...
func (s *Storage) GetURL(alias string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%s: failed to prepare the statement: %w", op, err)
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("%s: failed to execute the statement: %w", op, err)
	}
